...
export default function Epis() {
  ...
  const handleReceiptPDF = (emp: any, finalCart: any[], date: string) => {
    // Enrich cart items with full PPE data (like CA and Validity)
    const enrichedItems = finalCart.map(item => {
      const ppe = ppes.find(p => p.id === item.ppe_id);
      return {
        ...item,
        name: ppe?.name || 'EPI',
        ca: ppe?.ca || '-',
        validity_date: ppe?.validity_date || null
      };
    });

    const doc = generatePPEReceiptPDF(settings, emp, enrichedItems, date);
